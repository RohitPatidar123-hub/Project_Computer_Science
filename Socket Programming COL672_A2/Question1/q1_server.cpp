...
void handle_client(int c, const std::vector<std::string> &words,
                   int PACKET_SIZE) {
...
void accept_connections(int s, const std::vector<std::string> &words,
                        int PACKET_SIZE) {
  ...
  while (1) {
    ...
    handle_client(c, words, PACKET_SIZE);
    close(c);
  }
}
...
int main() {
  ...
  accept_connections(s, words, PACKET_SIZE);
