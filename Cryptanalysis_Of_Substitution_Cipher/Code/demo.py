...
class DecipherText(object): # Do not change thisch
    ...
    def decryptFromCiphertextToPlaintext(self, ciphertext, ignoreplainletter, cipherLetterToPlaintextMapping):
                ...
                for i in ciphertext:
                    ...
                    if i in escape_char or i not in cipherLetterToPlaintextMapping:
                        plaintext += i
                    elif  len(cipherLetterToPlaintextMapping[i])==0:
                           plaintext += "-"
    ...
    def getdecrypt(self, ciphertext,cltp):
         plaintext=" "
         for i in ciphertext.upper():
                if i in escape_char:
                    plaintext=plaintext+i
                elif i in cltp :
                       plaintext=plaintext+ cltp[i][0]
                else :
                     plaintext=plaintext+'-'
    ...
    # def maptocipher(self):
    #      cmt={'V': ['D'], 'Y': ['H'], '0': ['A'], '4': ['N'], 'P': ['F'], 'T': ['T'], 'X': ['R'], '8': ['I'], 'O': ['S'], 'N': ['W'], '5': ['B'], '#': ['E'], '7': ['O'], '3': ['W'], 'U': ['Y'], 'Q': ['M'], '1': ['C'], '9': ['F'], 'P': ['P'], '$': ['U'], 'Z': ['L'], '2': ['G'],'@':['K'],'a':['Z'],'a':['V'],'a':['Q'],'a':['J'],}
    #      return cmt


    def create_bidirectional_mapping(self, lastmap,ciphertext):
        """
        Given lastmap of the form:
        { 'PLAINWORD': ['PLAINWORD', 'CIPHERWORD'], ... }
        create two dictionaries:
        - lastplaintocipher: mapping each plaintext letter to a list of cipher letters.
        - lastciphertoplain: mapping each cipher letter to a list of plaintext letters.
        
        If a letter occurs multiple times, a set is maintained for uniqueness.
        """
        lastplaintocipher = self.getBlankPlainletterMapping()
        lastciphertoplain = self.getBlankCipherletterMapping(ciphertext)
        
        for plain_word, mapping in lastmap.items():
            # Assume mapping[0] is plain_word and mapping[1] is cipher_word
            cipher_word = mapping[0]
            
            # Ensure both words are of the same length
            if len(plain_word) != len(cipher_word):
                print(f"Warning: Word length mismatch for {plain_word} and {cipher_word}")
                continue
            
            # Use zip to iterate over corresponding letters
            for p, c in zip(plain_word, cipher_word):
                # Add to plain -> cipher mapping (using a set to ensure uniqueness)
                if p not in lastplaintocipher:
                    lastplaintocipher[p] = []
                if c not in  lastplaintocipher[p]:   
                   lastplaintocipher[p].append(c)
                
                # Add to cipher -> plain mapping
                if c not in lastciphertoplain:
                    lastciphertoplain[c] =[]
                if p not in lastciphertoplain[c]:    
                    lastciphertoplain[c].append(p)
        
        # # Optionally, convert sets to sorted lists for easier reading.
        # lastplaintocipher = {k: sorted(list(v)) for k, v in lastplaintocipher.items()}
        # lastciphertoplain = {k: sorted(list(v)) for k, v in lastciphertoplain.items()}
        # lastplaintocipher=self.removeSolvedLettersFromIntersectMapping(lastplaintocipher,ciphertext)
        # lastciphertoplain=self.removeSolvedLettersFromIntersectMapping(lastciphertoplain,ciphertext)
        return lastplaintocipher, lastciphertoplain
    ...
    def redefineCipherletterToPlaintextMapping(self,plaintext,ciphertext,ignoreplainletter,cipherletterToPlaintextMapping,plainletterToCiperletterMapping):
            ...
            for key, value in matching_words_from_semiplaintext_to_plaintext.items():
                    if len(value)==1:
                        ...
                        result=[cipherword]
                        lastmap[plainword]=[]
                        lastmap[plainword]=result      
                        # print(key," ",plainword," ",str ," " ,cipherword )
            ...
            print(lastmap)
            lastplaintocipher,lastciphertoplain=self.create_bidirectional_mapping(lastmap,ciphertext)
            print("lastplaintocipher: ",lastplaintocipher) 
            print(" lastciphertoplain: ",lastciphertoplain)
            ...
            # print(matching_words_from_semiplaintext_to_plaint
            return cipherletterToPlaintextMapping,lastplaintocipher, lastciphertoplain


    def getlastmethod(self, ciphertext,plaintext,cltp):
        plaintextmapp=self.getBlankPlainletterMapping()
        print(plaintextmapp)
        print(cltp)
        for key, value in cltp.items():
             plaintextmapp[value[0]].append(key)
        key1=''

        for  key,value in  plaintextmapp.items():
                if len(value)==0:
                  key1=key1+'A'
                else :
                   key1=key1+value[0] 
    ...
    def getkey(self, ciphertext,plaintext,cipherletterToPlaintextMapping,plainletterToCiperletterMapping):
            ...
            print("remaingCipherletter :",remaingCipherletter)
            if remaingCipherletter!='' and len(remaingCipherletter)==1:
    ...
    def DECRYPT(self, ciphertext , lastciphertoplain):
            plaintext=''
            print(lastciphertoplain)
            for i in ciphertext.upper():
                if i in escape_char:
                     plaintext=plaintext+i
                     
                     continue
                if len(lastciphertoplain[i])==1:
                       plaintext=plaintext+lastciphertoplain[i][0]
                else :
                     plaintext=plaintext+'-'
    ...
    def isstillincomplete(self, cipherletterToPlaintextMapping):
            status=bool
            for key,value in cipherletterToPlaintextMapping.items() :
                if(len(cipherletterToPlaintextMapping[key]!=1)):
                        status=True
            return True            
    def maptocipher(self):
         cmt={'V': ['D'], 'Y': ['H'], '0': ['A'], '4': ['N'], 'P': ['F'], 'T': ['T'], 'X': ['R'], '8': ['I'], 'O': ['S'], 'N': ['W'], '5': ['B'], '#': ['E'], '7': ['O'], '3': ['W'], 'U': ['Y'], 'Q': ['M'], '1': ['C'], '9': ['F'], 'P': ['P'], '$': ['U'], 'Z': ['L'], '2': ['G'],'@':['K'],'a':['Z'],'a':['V'],'a':['Q'],'a':['J'],}
    ...
    def getdecipheredtextanddecipheredkey(self ,ciphertext):
        ...
        # print("oldcipherletterToPlaintextMapping : ",len(oldcipherletterToPlaintextMapping))
        # for key in sorted(oldcipherletterToPlaintextMapping.keys()):
        #          print(f"{key}: {oldcipherletterToPlaintextMapping[key]}")
        count=3
        lastplaintocipher={}
        lastciphertoplain={}
        # print(plaintext)
        ...
        while(count!=0):
                ...
                cipherletterToPlaintextMapping,lastplaintocipher, lastciphertoplain = self.redefineCipherletterToPlaintextMapping(plaintext,ciphertext,ignoreplainletter,cipherletterToPlaintextMapping,plainletterToCiperletterMapping)
                # cipherletterToPlaintextMapping=self.getIntersectMappings(oldcipherletterToPlaintextMapping,oldcipherletterToPlaintextMapping,ciphertext)
                count=count-1

        PLAINTEXT=self.DECRYPT(ciphertext , lastciphertoplain)   
        # print("\n\nPLAINTEXT : ",PLAINTEXT,"\n\n")
        ...
        # print("lastciphertoplain :",lastciphertoplain,len(lastciphertoplain),"\n\n\n")
        # print("lastplaintocipher :",lastplaintocipher,len(lastplaintocipher))

        if(len(key)<26):
                cltp=self.maptocipher()
               
                
                plaintext=self.getdecrypt(ciphertext,cltp)
                # key,cipherletterToPlaintextMapping,plainletterToCiperletterMapping=self.getkey(ciphertext,plaintext,cltp,plainletterToCiperletterMapping)
                print("key :" ,key ,len(key))
                print(plaintext)
                key=self.getlastmethod(ciphertext,plaintext,cltp)
    ...
    def decipher(self, ciphertext): # Do not change this
        ...
        return deciphered_text.lower(), deciphered_key.lower() # Do not change this
...
if __name__ == '__main__': # Do not change this
    ...
    solve = DecipherText()
    deciphered_text, deciphered_key=solve.decipher(cipher_text8)
