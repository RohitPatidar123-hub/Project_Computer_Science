            ...
            for DELAY in delay_list:
                for FAST_RECOVERY in [True, False]:
                    for i in range(NUM_ITERATIONS):
                        ...
                        server_command = (
                            f"python3 -u Q1_server.py {server_ip} {SERVER_PORT} {int(FAST_RECOVERY)} "
                            f"> server.log 2>&1 &"
                        )
                        ...
                        client_command = (
                            f"python3 -u Q1_client.py {server_ip} {SERVER_PORT} > client.log 2>&1 "
                        )
                        ...
                        while not transfer_completed:
                            ...
                            client_process = h2.cmd("ps aux | grep 'Q1_client.py' | grep -v grep")
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python3 p1_exp.py <expname>")
    else:
